...
package FlowPlugin::ECTelegramRESTClient;
use strict;
use warnings;
use LWP::UserAgent;
use JSON;
...
use FlowPDF::Log;
## DO NOT EDIT THIS BLOCK === rest client imports ends, checksum: 8755ffd2385c1d2e8e8a00ee2c08b268 ===
# Place for the custom user imports, e.g. use File::Spec
## DO NOT EDIT THIS BLOCK === rest client starts ===
=head1

FlowPlugin::ECTelegramRESTClient->new('http://endpoint', {
...
use constant {
    ...
    USER_AGENT => 'EC-Telegram Flow Plugin',
...
Use this method to create a new FlowPlugin::ECTelegramRESTClient, e.g.

    my $client = FlowPlugin::ECTelegramRESTClient->new($endpoint,
...
# disable_notification: in body

sub sendSticker {
    my ($self, %params) = @_;

    $self->{method} = 'sendSticker';
    $self->{methodParameters} = \%params;

    my $uri = "/bot((token))/sendSticker";
...
## DO NOT EDIT THIS BLOCK === rest client ends, checksum: 60dc5e105085ca017a51a70b609c453b ===
=pod
...
sub augmentRequest {
    ...
    die "No token found in the client object" unless $token;
    my $uri = $r->uri;
    $uri =~ s/\(\(token\)\)/$token/g;
    $r->uri($uri);
